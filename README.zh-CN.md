# 软件清单

[English](README.md) | 简体中文

本机(Windows)已安装软件的分类清单,每条含一句话简介与官网或官方仓库链接。

## 目录

- [开发工具](#开发工具)
- [终端与网络](#终端与网络)
- [系统与效率](#系统与效率)
- [文件、压缩与文档](#文件压缩与文档)
- [自动化](#自动化)
- [媒体、图像与音乐](#媒体图像与音乐)
- [沟通与社交](#沟通与社交)
- [云盘与同步](#云盘与同步)
- [科学与地理信息](#科学与地理信息)
- [个人数据](#个人数据)
- [范围与整理方法](#范围与整理方法)
- [许可](#许可)

## 开发工具

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| Android Studio | 基于 IntelliJ IDEA 的官方安卓应用开发 IDE。 | [developer.android.com](https://developer.android.com/studio) |
| CC Switch | 切换 Claude Code、Codex 等 AI CLI 供应商配置的跨平台桌面工具。 | [github.com/farion1231](https://github.com/farion1231/cc-switch) |
| IntelliJ IDEA | JetBrains 出品的 JVM 语言 IDE,社区版免费。 | [jetbrains.com/idea](https://www.jetbrains.com/idea/) |
| Microsoft VS Code | 可扩展的代码编辑器,内置 Git、调试器与扩展市场。 | [code.visualstudio.com](https://code.visualstudio.com/) |
| 微信web开发者工具 | 开发微信小程序与公众号的官方 IDE。 | [developers.weixin.qq.com](https://developers.weixin.qq.com/miniprogram/dev/devtools/download.html) |
| WebStorm | JetBrains 出品的 JavaScript / TypeScript IDE。 | [jetbrains.com/webstorm](https://www.jetbrains.com/webstorm/) |

## 终端与网络

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| NekoBox(NekoRay) | 基于 sing-box 的 Qt 代理客户端,上游仓库已归档停止维护。 | [github.com/MatsuriDayo](https://github.com/MatsuriDayo/nekoray) |
| Watt Toolkit(Steam++) | 面向 Steam、GitHub 等服务的网络加速与脚本工具箱。 | [steampp.net](https://steampp.net/) |
| Xshell | Windows 上的 SSH / Telnet 终端客户端,家庭版免费。 | [xshell.com](https://www.xshell.com/) |

## 系统与效率

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| BleachBit | 跨平台磁盘与隐私清理工具,可清除缓存、日志与使用记录。 | [bleachbit.org](https://www.bleachbit.org/) |
| ContextMenuManager | 管理 Windows 右键菜单项的便携工具。 | [github.com/BluePointLilac](https://github.com/BluePointLilac/ContextMenuManager) |
| Dism++ | 基于 DISM 的便携系统维护与清理工具。 | [github.com/Chuyu-Team](https://github.com/Chuyu-Team/Dism-Multi-language) |
| Ditto | 开源剪贴板历史管理工具,支持搜索历史条目。 | [github.com/sabrogden](https://github.com/sabrogden/Ditto) |
| Geek Uninstaller | 便携卸载工具,可一并清除残留文件与注册表项。 | [geekuninstaller.com](https://geekuninstaller.com/) |
| PixPin | 集截图、贴图、长截图、OCR 与录屏于一体的工具。 | [pixpin.com](https://pixpin.com/) |
| Q-Dir | 四窗格文件管理器,支持标签页与快速筛选视图。 | [q-dir.com](https://www.q-dir.com/) |
| SpaceSniffer | 以矩形树图展示磁盘空间占用。 | [uderzo.it](http://www.uderzo.it/main_products/space_sniffer/) |

## 文件、压缩与文档

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| AnyTXT Searcher | 本地文档全文搜索引擎。 | [anytxt.net](https://anytxt.net/) |
| Bandizip | 支持 ZIP、7Z、RAR 等格式的快速压缩软件。 | [bandisoft.com](https://www.bandisoft.com/bandizip/) |
| PDF24 Creator | 免费的离线 PDF 工具箱,可创建、合并、压缩与编辑。 | [pdf24.org](https://www.pdf24.org/en/) |
| WPS Office | 含文字、表格、演示与 PDF 的办公套件。 | [wps.com](https://www.wps.com/) |

## 自动化

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| AutoHotkey | Windows 自动化脚本语言,用于编写热键与自动操作。 | [autohotkey.com](https://www.autohotkey.com/) |
| Autovisor | 基于 Playwright 的网课自动播放脚本。 | [github.com/CXRunfree](https://github.com/CXRunfree/Autovisor) |
| Microsoft-Rewards-Script | 基于 TypeScript 与 Playwright 的 Microsoft Rewards 每日任务自动化脚本。 | [github.com/TheNetsky](https://github.com/TheNetsky/Microsoft-Rewards-Script) |

## 媒体、图像与音乐

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| DaVinci Resolve | 集剪辑、调色、特效与音频后期于一体的专业视频软件。 | [blackmagicdesign.com](https://www.blackmagicdesign.com/products/davinciresolve) |
| NetEase Cloud Music(网易云音乐) | 带个性化推荐与社交功能的音乐客户端。 | [music.163.com](https://music.163.com/) |
| OBS Studio | 开源直播与录屏软件。 | [obsproject.com](https://obsproject.com/) |
| pineapple-pictures | 基于 Qt 的轻量图片查看器,含基础编辑。 | [github.com/BLumia](https://github.com/BLumia/pineapple-pictures) |
| PotPlayer | Daum 出品、功能丰富的全能播放器。 | [potplayer.daum.net](https://potplayer.daum.net/) |

## 沟通与社交

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| QQ | 腾讯即时通讯客户端,支持文件传输与群聊。 | [im.qq.com](https://im.qq.com/) |
| 微信(WeChat) | 腾讯的即时通讯客户端,支持支付与小程序。 | [weixin.qq.com](https://weixin.qq.com/) |
| WeLink | 华为云企业协同办公与视频会议客户端。 | [huaweicloud.com](https://www.huaweicloud.com/product/welink.html) |

## 云盘与同步

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| 百度网盘(Baidu Netdisk) | 百度云存储客户端。 | [pan.baidu.com](https://pan.baidu.com/) |
| 夸克网盘(Quark Cloud Drive) | 夸克云存储桌面客户端。 | [pan.quark.cn](https://pan.quark.cn/) |

## 科学与地理信息

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| QGIS | 开源地理信息系统,用于查看与分析空间数据。 | [qgis.org](https://qgis.org/) |

## 个人数据

| 软件 | 一句话简介 | 官网 / 官方仓库 |
|---|---|---|
| LifeLog | 本地优先的个人生活记录库,支持标签、全文搜索与 Markdown / Excel 导出。 | [github.com/Zzz210s](https://github.com/Zzz210s/LifeLog) |

## 范围与整理方法

本清单来自 2026-09-15 对 `E:\` 盘(本机 Windows 软件盘)顶层的扫描结果,一行对应一个已安装程序。

以下非软件项已排除:`$RECYCLE.BIN`、`Config.Msi`、`System Volume Information`(系统数据),以及 `0-cargo-target`(Rust 构建产物目录)。

所有链接在整理时都做过可达性检查;没有公开主页的条目改为链接其官方仓库。

## 许可

[MIT](LICENSE)
